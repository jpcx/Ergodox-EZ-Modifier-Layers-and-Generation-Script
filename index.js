...
//
// Modify the constants AUTHORIZATION, ALPHANUM_LAYOUT, and MODIFIER_LAYERS
...
(async () => {
  ...
  const getAlphaNumBodies = (hashId, modifiers = null) =>
      ALPHANUM_LAYOUT.reduce((bodies, code, idx) => {
        if (code)
          bodies.push({
            "operationName" : null,
            "variables" : {
  ...
  // Returns a Promise of POST requests for all of the alphanum keys on a layout
  const applyLayerKeys = (hashId, modifiers = null) => Promise.all(
      getAlphaNumBodies(hashId, modifiers)
          .map(
